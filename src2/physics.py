...
from utils import quat_conjugate, quat_from_angular_velocity, quat_from_axis_angle, quat_mul, quat_normalize, quat_rotate
...
@ti.kernel
def perturb_contacts(nc: ti.i32):
    PERTURB_ANGLE = 0.01
    DEDUP_TOL = 0.01
    ...
    for ci in range(nc):
        ...
        if ti.abs(n[1]) < 0.9:
            t0 = ti.Vector([0.0, 1.0, 0.0]).cross(n)
        else:
            t0 = ti.Vector([1.0, 0.0, 0.0]).cross(n)
        t0 = t0.normalized()
        t1 = n.cross(t0)

        # Save original transform
        orig_pos = data.geoms[gi_a].world_pos
        orig_quat = data.geoms[gi_a].world_quat

        # 4 perturbation directions
        for p in ti.static(range(4)):
            sign0 = 1.0 if p % 2 == 0 else -1.0
            sign1 = 1.0 if p < 2 else -1.0
            axis = (sign0 * t0 + sign1 * t1).normalized()

            # Rotate geom A around pivot
            dq = quat_from_axis_angle(axis, PERTURB_ANGLE)
            data.geoms[gi_a].world_quat = quat_normalize(quat_mul(dq, orig_quat))
            # Rotate position around pivot: new_pos = pivot + dq * (orig_pos - pivot)
            data.geoms[gi_a].world_pos = pivot + quat_rotate(dq, orig_pos - pivot)

            is_col, new_normal, new_pos, new_pen = mpr_collide(gi_a, data.contacts[ci].geom_b)

            if is_col == 1:
                # Dedup: check against all existing contacts
                is_dup = False
                existing_nc = data.num_contacts[None]
                for j in range(existing_nc):
                    if (data.contacts[j].pos - new_pos).norm() < DEDUP_TOL:
                        is_dup = True
                if not is_dup:
                    idx = ti.atomic_add(data.num_contacts[None], 1)
                    if idx < data.contacts.shape[0]:
                        data.contacts[idx].pos = new_pos
                        data.contacts[idx].normal = new_normal
                        data.contacts[idx].penetration = new_pen
                        data.contacts[idx].geom_a = gi_a
                        data.contacts[idx].geom_b = data.contacts[ci].geom_b

        # Restore original transform
        data.geoms[gi_a].world_pos = orig_pos
        data.geoms[gi_a].world_quat = orig_quat
