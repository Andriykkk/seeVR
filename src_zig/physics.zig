...
pub const Physics = struct {
    ...
    pub fn init(vk: *Vulkan, data: *const Data, allocator: std.mem.Allocator) !Physics {
        ...
        const solver_buffers = [16]Vulkan.Buffer{
            data.body_pos,              // 0
            data.body_quat,             // 1
            data.body_vel,              // 2
            data.body_omega,            // 3
            ...
            data.contact_lambda_n,      // 14
            data.atomic_counters,       // 15
        };
        const solver_pipe = try vk.createComputePipeline(solver_shader, 16, &solver_buffers, @sizeOf(SolverPC));

        return .{ .vk = vk, .physics_pipe = physics_pipe, .narrow_pipe = narrow_pipe, .solver_pipe = solver_pipe };
    }

    /// Full physics step: transforms → AABBs → broad phase → narrow phase (MPR) → solver → integrate
    pub fn step(self: *Physics, num_bodies: u32, num_geoms: u32, dt: f32, gravity: [3]f32) !void {
        ...
        // Step 1: apply gravity + PGS solve (serial, 1 thread)
        pushSolver(cmd, self.solver_pipe.layout, .{ .step = 1, .count = num_bodies, .dt = dt });
        c.vkCmdDispatch(cmd, 1, 1, 1);
        c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);
