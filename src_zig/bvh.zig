...
pub const BVH = struct {
    ...
    pub fn init(vk: *Vulkan, data: *const Data, allocator: std.mem.Allocator) !BVH {
        const shader = try vk.getShader("shaders/bvh.comp", .compute, allocator);

        // 16 buffer bindings matching bvh.comp layout
        const buffers = [16]Vulkan.Buffer{
            data.vertices,          // 0
            data.indices,           // 1
            data.bvh_tri_centroids, // 2
            data.bvh_morton_codes,  // 3
            data.bvh_morton_temp,   // 4
            data.bvh_sort_indices,  // 5
            data.bvh_sort_temp,     // 6
            data.bvh_scene_bounds,  // 7
            data.bvh_nodes_min,     // 8
            data.bvh_nodes_max,     // 9
            data.bvh_nodes_left,    // 10
            data.bvh_nodes_right,   // 11
            data.bvh_nodes_count,   // 12
            data.bvh_nodes_parent,  // 13
    ...
    /// Build LBVH on GPU. Records all dispatches into a one-shot command buffer and submits.
    pub fn build(self: *BVH, num_tris: u32) !void {
        ...
        // Step 0: centroids + init sort indices
        self.pushAndDispatch(cmd, .{ .step = 0, .num_tris = num_tris, .pass_shift = 0 }, groups);
        ...
        // Step 1: scene bounds
        self.pushAndDispatch(cmd, .{ .step = 1, .num_tris = num_tris, .pass_shift = 0 }, 1);
        c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);

        // Step 2: morton codes
        self.pushAndDispatch(cmd, .{ .step = 2, .num_tris = num_tris, .pass_shift = 0 }, groups);
        c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);

        // Step 3: radix sort (4 passes)
        for ([_]u32{ 0, 8, 16, 24 }) |shift| {
            self.pushAndDispatch(cmd, .{ .step = 3, .num_tris = num_tris, .pass_shift = shift }, 1);
            c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);
        }

        // Step 4: build hierarchy
        self.pushAndDispatch(cmd, .{ .step = 4, .num_tris = num_tris, .pass_shift = 0 }, groups);
        c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);

        // Step 5: leaf AABBs
        self.pushAndDispatch(cmd, .{ .step = 5, .num_tris = num_tris, .pass_shift = 0 }, groups);
        c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);

        // Step 6: clear flags
        self.pushAndDispatch(cmd, .{ .step = 6, .num_tris = num_tris, .pass_shift = 0 }, groups);
        c.vkCmdPipelineBarrier(cmd, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, c.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, null, 0, null);

        // Step 7: propagate AABBs
        self.pushAndDispatch(cmd, .{ .step = 7, .num_tris = num_tris, .pass_shift = 0 }, groups);
    ...
    fn pushAndDispatch(self: *const BVH, cmd: c.VkCommandBuffer, pc: PushConstants, groups: u32) void {
