...
pub fn main() !void {
    ...
    // Ground (static)
    _ = try d.addBox(.{ 0, -0.25, 0 }, .{ 10, 0.25, 10 }, .{ 0.3, 0.3, 0.35 }, 0, 0.5);
    // Stacked boxes
    _ = try d.addBox(.{ 0, 1, 0 }, .{ 0.5, 0.5, 0.5 }, .{ 0.85, 0.25, 0.25 }, 1, 0.5);
    _ = try d.addBox(.{ 0.02, 2.5, 0 }, .{ 0.5, 0.5, 0.5 }, .{ 0.25, 0.55, 0.85 }, 1, 0.5);
    _ = try d.addBox(.{ -0.02, 4.0, 0 }, .{ 0.5, 0.5, 0.5 }, .{ 0.85, 0.65, 0.2 }, 1, 0.5);
    _ = try d.addBox(.{ 0.01, 5.5, 0 }, .{ 0.5, 0.5, 0.5 }, .{ 0.35, 0.8, 0.35 }, 1, 0.5);
    // Side boxes
    _ = try d.addBox(.{ 3, 1, 0 }, .{ 0.75, 0.75, 0.75 }, .{ 0.3, 0.8, 0.3 }, 1, 0.3);
    _ = try d.addBox(.{ -3, 2, 1 }, .{ 0.6, 0.6, 0.6 }, .{ 0.8, 0.5, 0.2 }, 1, 0.8);
    // Spheres
    _ = try d.addSphere(.{ 1.5, 3, 0 }, 0.4, .{ 0.9, 0.3, 0.9 }, 12, 1, 0.2);
    _ = try d.addSphere(.{ -1.0, 3, 0 }, 0.5, .{ 0.3, 0.9, 0.9 }, 12, 1, 0.6);
